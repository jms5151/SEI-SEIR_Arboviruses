...
K_thr_briere_reduceK <- function(temp, rain, Rmax, N, timestep, percReduce){
  ...
  if((rain < R0) | (rain > Rmax)){
    max(0.01*carrying_capacity_th(temp, 100, 29.0, 0.05, N, timestep), 1000) * (1-percReduce)
  }
  else {
    c <- 7.86e-05
    max(carrying_capacity_th(temp, 100, 29.0, 0.05, N, timestep)*c*rain*(rain-R0)*sqrt(Rmax-rain)*0.268 + 0.001, 1000) * (1-percReduce)
  }
}
...
K_thr_quadratic_reduceK <- function(temp, rain, Rmax, N, timestep, percReduce){
  ...
  if((rain < R0) | (rain > Rmax)){
    ...
    c <- -5.99e-03
    max(carrying_capacity_th(temp, 100, 29.0, 0.05, N, timestep)*(c*(rain-R0)*(rain-Rmax))*0.045 + 0.001, 1000) * (1-percReduce)
  }
}
